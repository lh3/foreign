...
{
	int i, j, dec, pop, pop2, c1, c2, ind, rchrom, hot ;
	int migrant, source_pop, *config, flagint ;
	double  ran1(), sum, x, ttemp, rft, clefta,  crighta, tmin, p  ;
...
{
	struct seg *pseg ;
	int  hot, lsg, lsgm1,  ic,  is, i;
	double ran1(), x, sum, len, ic_sum;
	int xover(int, int, int);
...
int crightr_V(int nsam)
{
	struct seg *pseg ;
	int   hot, lsg, lsgm1, ic, is, i;
	double ran1(), x, sum, len, rando, ic_sum;
	int xover(int, int, int);
...
{
	struct seg *pseg ;
	int len_left, len_right, el, lsg, lsgm1,  ic,  is, spot, begic, endic ;
	double ran1();
	int xover(), ca() ;
	int leftgc = 1;
	int rightgc = 1;
...
{
	struct seg *pseg ;
	int hot, len_left, len_right, el, lsg, lsgm1, ic, is, begic, endic ;
	double ran1();
	int xover(), ca() ;
