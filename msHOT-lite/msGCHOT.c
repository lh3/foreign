...
int main(int argc, char *argv[])
{
    ...
    while( howmany-count++ ) {
        ...
        if( (segsites > 0 ) || ( pars.mp.theta > 0.0 ) ) {
			if (pars.cp.is_lh3 && segsites > 0 && pars.cp.nsites > 0) {
				int last_pos = 0;
				fprintf(pf, "@begin %d\n%d\n", segsites, pars.cp.nsites);
				for (i = 0; i != segsites; ++i) {
					int pos = (int)(posit[i] * pars.cp.nsites);
					if (pos != last_pos) {
						int j;
						fprintf(pf, "%d\t", pos);
...
getpars(int argc, char *argv[], int *phowmany )
{
	...
	while( arg < argc ){
		...
		switch ( argv[arg][1] ){
		case 'l': ++arg; pars.cp.is_lh3 = 1; break;
		case 'f' :
...
int usage()
{
	...
	fprintf(stderr,"\t -l   (Switch on lh3 specific output.)\n"); 
...
parens( struct node *ptree, int *descl, int *descr,  int noden)
{
	...
	if( descl[noden] == -1 ) {
		printf("%d:%7.5lf", noden+1, (ptree+ ((ptree+noden)->abv))->time );
	}
	else{
		printf("(");
